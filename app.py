...
def load_model():
    ...
    if not os.path.exists(config.MODEL_SAVE_PATH):
        return False, "모델 파일을 찾을 수 없습니다. 먼저 train.py를 실행하여 모델을 학습하세요."
    ...
    try:
        ...
        if os.path.exists(config.SCALER_SAVE_PATH):
            ...
            return False, "Scaler 파일을 찾을 수 없습니다."
        
        # feature_names 로드 (scaler와 함께 저장됨)
        # feature_names가 없으면 데이터 파일에서 다시 로드
        if not data_processor.feature_names:
            try:
                # 파일 경로 확인
                csv_file_path = config.TRAIN_ACTIVITY_PATH
                if not os.path.exists(csv_file_path):
                    # 절대 경로로 시도
                    csv_file_path = os.path.join(os.getcwd(), csv_file_path)
                
                if os.path.exists(csv_file_path):
                    temp_df = pd.read_csv(csv_file_path, nrows=1, encoding='utf-8')
                    numeric_cols = temp_df.select_dtypes(include=[np.number]).columns.tolist()
                    exclude_keywords = ["time", "date", "timestamp", "id", "user_id", "email"]
                    data_processor.feature_names = [
                        col for col in numeric_cols 
                        if not any(keyword in col.lower() for keyword in exclude_keywords)
                    ]
                    print(f"feature_names 자동 로드: {len(data_processor.feature_names)}개 특징")
                else:
                    # 파일이 없으면 기본 특징명 사용 (모델의 input_size 기반)
                    print(f"경고: 데이터 파일을 찾을 수 없습니다. 기본 특징 수 사용: {saved_input_size}")
                    data_processor.feature_names = [f"feature_{i}" for i in range(saved_input_size)]
            except Exception as e:
                # 파일 로드 실패 시 기본 특징명 사용
                print(f"경고: feature_names 로드 실패 ({str(e)}). 기본 특징 수 사용: {saved_input_size}")
                data_processor.feature_names = [f"feature_{i}" for i in range(saved_input_size)]
        
        # Anomaly Detector 생성
        anomaly_detector = AnomalyDetector(model)
        
        # 임계값 계산 (validation 데이터 사용)
        try:
            # Validation 데이터 로드
            val_file_path = config.VAL_ACTIVITY_PATH
            if not os.path.exists(val_file_path):
                val_file_path = os.path.join(os.getcwd(), val_file_path)
            
            if os.path.exists(val_file_path):
                print("Validation 데이터를 로드하여 임계값을 계산합니다...")
                # Validation 데이터 로드 및 전처리
                val_df = pd.read_csv(val_file_path, encoding='utf-8')
                val_df = data_processor.handle_missing_values(val_df)
                
                # 특징 선택
                val_features = val_df[data_processor.feature_names].values
                
                # 정규화 (이미 학습된 scaler 사용)
                val_normalized = data_processor.normalize(val_features, fit=False)
                
                # 시계열 시퀀스 생성
                sequence_length = config.MODEL_CONFIG["sequence_length"]
                X_val = []
                for i in range(len(val_normalized) - sequence_length):
                    X_val.append(val_normalized[i:i + sequence_length])
                X_val = np.array(X_val)
                
                # 임계값 계산
                multiplier = config.ANOMALY_CONFIG["threshold_multiplier"]
                use_percentile = config.ANOMALY_CONFIG.get("use_percentile", False)
                percentile = config.ANOMALY_CONFIG.get("percentile", 95)
                min_threshold = config.ANOMALY_CONFIG.get("min_threshold", 0.01)
                threshold = anomaly_detector.compute_threshold(
                    X_val, 
                ...
                print(f"최종 임계값: {threshold:.4f}")
            else:
                # Validation 데이터가 없으면 기본 임계값 사용
                min_threshold = config.ANOMALY_CONFIG.get("min_threshold", 0.01)
                print(f"경고: Validation 데이터 파일을 찾을 수 없습니다. 기본 임계값 사용: {min_threshold:.4f}")
                anomaly_detector.threshold = min_threshold
        except Exception as e:
            # 임계값 계산 실패 시 기본값 사용
            min_threshold = config.ANOMALY_CONFIG.get("min_threshold", 0.01)
            print(f"경고: 임계값 계산 실패 ({str(e)}). 기본 임계값 사용: {min_threshold:.4f}")
            anomaly_detector.threshold = min_threshold
        ...
        if len(sensor_data) < sequence_length:
            ...
            for i in range(padding_needed):
                sensor_data.insert(0, {
                    "time": sensor_data[0]["time"] if sensor_data else datetime.now().isoformat()[:16],
                    "heart_rate": 0,
                    "steps": 0,
                    "sleep": 0,
                    "temperature": 0,
...
def send_emergency_alert():
    ...
    try:
        ...
        # 긴급 알림 전송 (수동 응급 연락이므로 is_manual_emergency=True)
        ...
        if success:
            ...
            return jsonify({
                "success": False,
                "error": "긴급 알림 전송에 실패했습니다. 이메일 설정 및 긴급 연락망을 확인해주세요."
            }), 500
